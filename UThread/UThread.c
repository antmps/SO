...
//
// A list to save threads that have been created but not terminated.
//
static
...
VOID UtRun () {
	...
	//
	// Change RunningThread state before changing to the MainThread.
	//
...
VOID UtYield () {
	if (!IsListEmpty(&ReadyQueue)) {
		...
		InsertTailList(&AliveQueue, &RunningThread->Link);
		RunningThread->State = "Ready";
		Schedule();
	}
}
...
VOID UtActivate (HANDLE ThreadHandle) {
	...
	InsertTailList(&AliveQueue, &((PUTHREAD)ThreadHandle)->Link);
...
DWORD UtJoin(HANDLE thread){
	...
	if (!UtAlive(thread)) return 0;

	if (&((PUTHREAD)thread)->BlockedList == NULL){
		InitializeListHead(&((PUTHREAD)thread)->BlockedList);
	}

	InsertTailList((&((PUTHREAD)thread)->BlockedList),&RunningThread->Link );
	RunningThread->State = "Blocked";
	Schedule();
